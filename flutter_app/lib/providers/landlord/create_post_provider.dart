...
class CreatePostProvider with ChangeNotifier {
  ...
  final promotionalOffersController = TextEditingController();
  final promotionalperiodStartController = TextEditingController();
  final promotionalperiodEndController = TextEditingController();
  final parkingspacesController = TextEditingController();

  String roomType = 'Phòng trọ';
  ...
  Future<bool> createPost(BuildContext context) async {
    ...
    try {
      ...
      await roomRef.set({
        ...
        'promotionalOffers': promotionalOffersController.text.trim(),
        'promotionalperiodStart': promotionalperiodStartController.text.trim().isNotEmpty
            ? DateTime.parse(promotionalperiodStartController.text.trim())
            : DateTime.now(),
        'promotionalperiodEnd': promotionalperiodEndController.text.trim().isNotEmpty
            ? DateTime.parse(promotionalperiodEndController.text.trim())  
            : DateTime.now(),
        'parkingspaces': int.tryParse(parkingspacesController.text) ?? 0,
  ...
  void _reset() {
    ...
    promotionalperiodStartController.clear();
    promotionalperiodEndController.clear();
    depositController.clear();
    promotionalOffersController.clear();
  ...
  void dispose() {
    ...
    promotionalOffersController.dispose();
    promotionalperiodEndController.dispose();
    floorController.dispose();
    areaController.dispose();
    priceController.dispose();
    descriptionController.dispose();
    promotionalperiodStartController.dispose();
    super.dispose();
  }
}
